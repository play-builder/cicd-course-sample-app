...
import { readImageBlock, setImageInFile } from './gitops-values-lib.mjs';
...
if (command === 'set' && args.length === 3) {
  ...
  console.log(`updated ${fileName} -> ${repository}@${digest}`);
  ...
  setImageInFile(prodFile, devImage.repository, devImage.digest);
  console.log(`promoted ${devImage.repository}@${devImage.digest} to ${prodFile}`);
} else {
